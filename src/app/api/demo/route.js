...
const companyData = {
  ...
  superadmin: { id: "super1", email: "super@admin.com", password: "1234", role: "superadmin", name: "Super Admin" },
  hrs: [
  ...
  employees: [
    ...
    { id: "emp2", email: "emp2@techcorp.com", password: "1234", role: "employee", name: "Reshma" },
  ],
};
...
export async function POST(req) {
  try {
    ...
    if (email === companyData.superadmin.email && password === companyData.superadmin.password) {
      return NextResponse.json({ ...companyData.superadmin, companyId: companyData.companyId, companyName: companyData.companyName });
    }

    // HR
    const hr = companyData.hrs.find(u => u.email === email && u.password === password);
    if (hr) {
      return NextResponse.json({ ...hr, companyId: companyData.companyId, companyName: companyData.companyName });
    }

    // Employee
    const emp = companyData.employees.find(u => u.email === email && u.password === password);
    if (emp) {
      return NextResponse.json({ ...emp, companyId: companyData.companyId, companyName: companyData.companyName });
    }
