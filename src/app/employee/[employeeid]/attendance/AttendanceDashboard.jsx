  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        let attendanceData = await getMyAttendance();

        // =============================
        // ✅ SAFETY: ensure it's always an array
        // =============================
        if (!Array.isArray(attendanceData)) {
          console.warn("Attendance API did not return an array:", attendanceData);
          attendanceData = [];
        }
        ...
        const join = employee?.joining_date ? toMidnight(new Date(employee.joining_date)) : null;
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <Button variant="outline" size="icon" onClick={() => setCurrentMonth(p => new Date(p.getFullYear(), p.getMonth() - 1, 1))}>
                  <ChevronLeft className="h-5 w-5" />
                </Button>

                <div className="px-3 font-semibold text-base">{MONTHS[currentMonth.getMonth()]} {currentMonth.getFullYear()}</div>
            ...
            <CardContent>
              {loading ? (
                <div className="flex items-center justify-center h-[320px]"><Spinner /></div>
              ) : (
                <div className="grid grid-cols-7 border rounded-md text-sm">
                  {WEEKDAYS.map(day => (
                    <div key={day} className="p-2 border-b text-center font-semibold bg-muted">{day}</div>
                  ))}

                  {calendarRows.flat().map(cell => (
                    <div key={cell.key} className="relative h-[85px] border p-2">
                      {cell.isCurrentMonth && (
                        <>
                          <div className={`text-sm font-semibold ${cell.isToday ? "bg-primary text-white px-2 py-0.5 rounded-full inline-block" : ""}`}>{cell.dayNumber}</div>

                          {cell.status && (
                            <Badge className={`absolute bottom-2 right-2 ${cell.status === "Absent" ? "bg-red-500 text-white" : "bg-emerald-500 text-white"}`}>
