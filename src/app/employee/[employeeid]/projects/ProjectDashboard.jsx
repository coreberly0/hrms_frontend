...
export default function ProjectDashboard({ employeeid }) {
  ...
  return (
    <div className="flex flex-col p-6 gap-4">
      ...
      {/* TABLE HEADER (FIXED) */}
      <div className="grid grid-cols-12 text-sm font-medium text-muted-foreground px-4 shrink-0">
        ...
        <div className="space-y-2">
          {paginatedProjects.length === 0 ? (
            ...
            paginatedProjects.map((p, index) => (
              <div
                key={p.id}
                className="grid grid-cols-12 items-center border rounded-lg px-4 py-4 bg-background hover:bg-gray-100 transition-colors"
              >
                {/* SERIAL */}
                ...
                <div className="col-span-3">
                  ...
                  <div className="text-xs text-muted-foreground">
                    ID: {p.id} • {new Date(p.created_at).toLocaleDateString()}
                  </div>
                </div>
                ...
                {role === "manager" && (
                  <div className="col-span-1 flex justify-end gap-2">
                    <Button size="icon" variant="outline" onClick={() => setEdit(p)}>
